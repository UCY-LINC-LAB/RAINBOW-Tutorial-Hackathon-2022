...
obj_interpreter = tflite.Interpreter(model_path=OBJ_MODEL, num_threads=4)
...
face_interpreter = tflite.Interpreter(model_path=FACE_MODEL, num_threads=4)
...
# Setup face detection
scene_interpreter = tflite.Interpreter(model_path=SCENE_MODEL, num_threads=4)
...
async def info():
    im1 = "There is no image yet"
    try:
        data_uri = base64.b64encode(open("latest.jpg", 'rb').read()).decode('utf-8')
        im1 = '<img src="data:image/png;base64,{0}">'.format(data_uri)
        f = open('data.json')
        data = json.load(f)
        im1 += "<p>"+str([prediction['label']+": "+str(prediction['confidence']) for prediction in data['predictions']])
    except Exception:
        pass
    return HTMLResponse(content=im1, status_code=200)
